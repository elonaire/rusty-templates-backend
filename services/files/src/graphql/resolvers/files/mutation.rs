...
impl FileMutation {
    pub async fn add_product_artifact(
        ...
        if let Some(headers) = ctx.data_opt::<HeaderMap>() {
            ...
            let internal_product =
                add_foreign_key_if_not_exists::<Product>(ctx, product_fk_body).await;
            let internal_license =
                add_foreign_key_if_not_exists::<License>(ctx, license_fk_body).await;
                ...
                Some(file_found) => {
                    let mut product_artifact_query = db
                        ...
                        .bind((
                            ...
                            format!(
                                "file:{}",
                                file_found.id.as_ref().map(|t| &t.id).expect("id").to_raw()
                            ),
                        ))
                        .await
                        .map_err(|e| Error::new(e.to_string()))?;
